...
module.exports = async (files) => {
  ...
  documentFile.data = await stringReplaceAsync(
    ...
    async (val) => {
      ...
      // and are not inserted (re-used) somewhere else loosing xml namespaces
      const drawingEl = new DOMParser().parseFromString(val)
      const chartDrawningEl = drawingEl.getElementsByTagName('c:chart')[0]
      ...
      const chartStyleRelFilename = `word/charts/${nodeListToArray(chartRelsDoc.getElementsByTagName('Relationship')).find((el) => {
        return el.getAttribute('Type') === 'http://schemas.microsoft.com/office/2011/relationships/chartStyle'
      }).getAttribute('Target')}`

      if (!originalChartsXMLMap.has(chartStyleRelFilename)) {
      ...
      const chartColorStyleRelFilename = `word/charts/${nodeListToArray(chartRelsDoc.getElementsByTagName('Relationship')).find((el) => {
        return el.getAttribute('Type') === 'http://schemas.microsoft.com/office/2011/relationships/chartColorStyle'
      }).getAttribute('Target')}`

      if (!originalChartsXMLMap.has(chartColorStyleRelFilename)) {
      ...
      if (drawingEl.documentElement.firstChild.nodeName === 'c:title') {
        const newChartTitleEl = drawingEl.documentElement.firstChild
        ...
        if (chartRId !== newChartRelId) {
          ...
          const newChartStyleId = files.filter((f) => /word\/charts\/style(\d+)\.xml/.test(f.path)).reduce((lastId, f) => {
          ...
          const newChartColorStyleId = files.filter((f) => /word\/charts\/colors(\d+)\.xml/.test(f.path)).reduce((lastId, f) => {
          ...
          const newChartStyleType = nodeListToArray(contentTypesDoc.getElementsByTagName('Override')).find((el) => {
          ...
          const newChartColorStyleType = nodeListToArray(contentTypesDoc.getElementsByTagName('Override')).find((el) => {
