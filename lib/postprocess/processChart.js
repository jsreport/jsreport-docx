...
module.exports = async function processChart (files, drawingEl, originalChartsXMLMap, newRelIdCounterMap) {
  ...
  for (const chartTitleTextEl of chartTitleTextElements) {
    ...
    if (chartDrawingEl.prefix === 'cx') {
      ...
      const chartTypeContentEl = chartPlotAreaEl.getElementsByTagName('c:layout')[0].nextSibling
      ...
      if (chartConfig.options != null) {
        if (chartType === 'scatterChart') {
          if (chartConfig.options.scales && Array.isArray(chartConfig.options.scales.xAxes) && chartConfig.options.scales.xAxes.length > 0) {
            const xAxisConfig = chartConfig.options.scales.xAxes[0]
            const xAxisEl = findChildNode('c:valAx', chartPlotAreaEl, true)[0]
            const scalingEl = findChildNode('c:scaling', xAxisEl)

            if (xAxisConfig.display != null) {
              const deleteEl = findChildNode('c:delete', xAxisEl)
              deleteEl.setAttribute('val', xAxisConfig.display === false ? 1 : 0)
            }

            if (xAxisConfig.ticks && xAxisConfig.ticks.stepSize != null) {
              const majorUnitEl = findOrCreateChildNode(chartDoc, 'c:majorUnit', xAxisEl)
              majorUnitEl.setAttribute('val', xAxisConfig.ticks.stepSize)
            }

            if (xAxisConfig.ticks && xAxisConfig.ticks.min != null) {
              const minEl = findOrCreateChildNode(chartDoc, 'c:min', scalingEl)
              minEl.setAttribute('val', xAxisConfig.ticks.min)
            }

            if (xAxisConfig.ticks && xAxisConfig.ticks.max != null) {
              const maxEl = findOrCreateChildNode(chartDoc, 'c:max', scalingEl)
              maxEl.setAttribute('val', xAxisConfig.ticks.max)
            }
          }

          if (chartConfig.options.scales && Array.isArray(chartConfig.options.scales.yAxes) && chartConfig.options.scales.yAxes.length > 0) {
            const yAxisConfig = chartConfig.options.scales.yAxes[0]
            const yAxisEl = findChildNode('c:valAx', chartPlotAreaEl, true)[1]
            const scalingEl = findChildNode('c:scaling', yAxisEl)

            if (yAxisConfig.display != null) {
              const deleteEl = findChildNode('c:delete', yAxisEl)
              deleteEl.setAttribute('val', yAxisConfig.display === false ? 1 : 0)
            }

            if (yAxisConfig.ticks && yAxisConfig.ticks.stepSize != null) {
              const majorUnitEl = findOrCreateChildNode(chartDoc, 'c:majorUnit', yAxisEl)
              majorUnitEl.setAttribute('val', yAxisConfig.ticks.stepSize)
            }

            if (yAxisConfig.ticks && yAxisConfig.ticks.min != null) {
              const minEl = findOrCreateChildNode(chartDoc, 'c:min', scalingEl)
              minEl.setAttribute('val', yAxisConfig.ticks.min)
            }

            if (yAxisConfig.ticks && yAxisConfig.ticks.max != null) {
              const maxEl = findOrCreateChildNode(chartDoc, 'c:max', scalingEl)
              maxEl.setAttribute('val', yAxisConfig.ticks.max)
            }
          }
        }
      }
