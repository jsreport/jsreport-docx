    ...
    if (chartDrawingEl.prefix === 'cx') {
      ...
      for (const [serieIdx, serieEl] of existingChartSeriesElements.entries()) {
        ...
        prepareChartSerie(chartDoc, chartType, newChartSerieNode, {
          ...
          dataLabels: chartConfig.data.labels,
...
function prepareChartSerie (chartDoc, chartType, baseChartSerieEl, serieData) {
  const { serieIdx, serieLabel, dataLabels, dataValues } = serieData
  ...
  if (chartType === 'scatterChart' || chartType === 'bubbleChart') {
    addChartSerieItem(chartDoc, { name: 'c:xVal', data: { values: dataLabels } }, baseChartSerieEl)
    ...
    addChartSerieItem(chartDoc, { name: 'c:cat', type: chartType, data: { values: dataLabels } }, baseChartSerieEl)
    addChartSerieItem(chartDoc, { name: 'c:val', data: { count: dataLabels.length, values: dataValues } }, baseChartSerieEl)
  }
}
...
function addChartSerieItem (docNode, nodeInfo, targetNode) {
  ...
  switch (nodeInfo.name) {
    ...
    case 'c:bubbleSize':
      ...
      ptCountNode.setAttribute('val', nodeInfo.data.count != null ? nodeInfo.data.count : nodeInfo.data.values.length)

      const existingPtNodes = findChildNode('c:pt', cacheNode, true)

      for (const [idx, item] of nodeInfo.data.values.entries()) {
        let ptNode
