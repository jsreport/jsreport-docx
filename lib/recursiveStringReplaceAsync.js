...
async function assignReplacement (match, replacer) {
  ...
  if (match.matches) {
    hasNestedMatch = true
    const newChildMatchesWithReplace = await concurrency(match.matches, replacer)
  ...
  return replacerFn(args).then(function (res) {
...
async function processString (str, left, right, flags, replacer) {
  const matches = matchAll(str, left, right, flags)

  return concurrency(matches, replacer).then(function (matches) {
