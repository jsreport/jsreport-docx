...
module.exports = (reporter, definition) => async (req, res) => {
  ...
  const relsFile = files.find(f => f.path === 'word/_rels/document.xml.rels')
  const relsDoc = new DOMParser().parseFromString(relsFile.data.toString())

  const contentTypesFile = files.find(f => f.path === '[Content_Types].xml')
  const contentTypesDoc = new DOMParser().parseFromString(contentTypesFile.data.toString())
  const types = contentTypesDoc.getElementsByTagName('Types')[0]
  const defaultPng = contentTypesDoc.createElement('Default')
  ...
  for (const f of files) {
    // it looks that for now it is enough just to transform document.xml file
    if (f.path.includes('word/document.xml')) {
      const preprocessed = preprocess(f.data.toString())
      const evaluated = await renderEngine(reporter, {
        data: req.data,
      ...
      f.data = Buffer.from(postprocess(evaluated, relsDoc, newFiles))
    }
  }

  relsFile.data = Buffer.from(new XMLSerializer().serializeToString(relsDoc))
  contentTypesFile.data = Buffer.from(new XMLSerializer().serializeToString(contentTypesDoc))
